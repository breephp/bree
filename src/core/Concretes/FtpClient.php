...
class FtpClient
{
    ...
    public function putFile(string $file, int $mode = FTP_ASCII, ?Ignore $ignore = null): void
    {
        if (! is_file($file)) {
        ...
        if (! ($path = $this->getPathOf($file, $ignore))) {
            return;
        }
    ...
    public function putDir(string $dir, int $mode = FTP_ASCII, ?Ignore $ignore = null): void
    {
        ...
        if (! ($path = $this->getPathOf($dir, $ignore))) {
            return;
        }
